...
public class SetMoneyCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(player.hasPermission("craftq.*")) {
			...
			if(args.length < 2) {
				player.sendMessage(Economy.pr + "§cVerwendung: /setmoney <Spieler> <Betrag>");
				return true;
			}
			...
			try {
				...
				player.sendMessage(Economy.pr + "§cUngültiger Betrag. Verwendung: /setmoney <Spieler> <Betrag>");
				return true;
			}
			...
			EconomyMySQLAPI.setCoins(receiverUUID, target.getName(), amount);
			
			player.sendMessage(Economy.pr + "§7Du hast erfolgreich das Geld von §e" + target.getName() + " §7auf §e" + amount + "§e€ §7gesetzt.");
